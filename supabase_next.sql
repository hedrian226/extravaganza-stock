...
-- 7) Manager activity history. This is additive and does not change the
-- existing stock/viewer RPCs. Managers can only read/write their own logs.
...
drop policy if exists "activity logs own rows" on public.activity_logs;
create policy "activity logs own rows"
